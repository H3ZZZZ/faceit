...
@Service
public class AdvancedEventService {
    ...
    public AdvancedEventService(@Qualifier("unauthenticatedWebClient") WebClient unauthenticatedWebClient,
    ...
    public GogoLanEventDTO getEvent(List<String> playerIds,
                                    LocalDate startDate,
                                    LocalDate endDate,
                                    String name) {
        List<String> trackedPlayerIds = playerIds == null
                ? List.of()
                : playerIds.stream()
        ...
        String eventName = name != null && !name.isBlank() ? name.trim() : "Custom Squad";
        String sortedIds = trackedPlayerIds.stream().sorted().collect(Collectors.joining(","));
        String cacheKey = eventName + "|" + startDate + "|" + endDate + "|" + sortedIds;
        ...
        trackedPlayerIds.parallelStream().forEach(playerId -> {
        ...
        List<EventPlayerStatsDTO> players = trackedPlayerIds.stream()
                ...
                .toList();
        Map<String, AdvancedStatsAccumulator> advancedStatsByPlayer = buildAdvancedStats(eventMatchesByPlayer, trackedPlayerIds);
    ...
    public GogoLanEventDTO getEvent(LocalDate startDateOverride, LocalDate endDateOverride) {
        ...
        List<EventPlayerStatsDTO> players = properties.getPlayerIds().stream()
                ...
                .sorted(Comparator.comparingInt(EventPlayerStatsDTO::getTotalEloGain).reversed()
                        .thenComparing(Comparator.comparingDouble(EventPlayerStatsDTO::getAvgKd).reversed()))
                .toList();
        Map<String, AdvancedStatsAccumulator> advancedStatsByPlayer = buildAdvancedStats(eventMatchesByPlayer, properties.getPlayerIds());
        players.forEach(player -> applyAdvancedStats(player, advancedStatsByPlayer.get(player.getPlayerId())));
    ...
    private List<EventAwardDTO> buildAwards(List<EventPlayerStatsDTO> players) {
        ...
        return List.of(
                award("Event MVP", maxByAll(active, Comparator.comparingDouble(this::mvpScore)), player -> String.format("%.1f", mvpScore(player)), "Composite score from ELO, K/D, ADR and win rate."),
                ...
                award("Ironman", maxByAll(active, Comparator.comparingInt(EventPlayerStatsDTO::getMatchesPlayed)), player -> player.getMatchesPlayed() + " matches", "Most games played in the selected window."),
    ...
    private Map<String, AdvancedStatsAccumulator> buildAdvancedStats(Map<String, List<MatchStats>> matchesByPlayer, List<String> trackedPlayerIds) {
        Set<String> trackedPlayers = new LinkedHashSet<>(trackedPlayerIds);
            ...
            response.getRounds().forEach(round -> {
                ...
                round.getTeams().forEach(team -> {
                    ...
                    team.getPlayers().forEach(player -> {
                        if (!trackedPlayers.contains(player.getPlayerId())) {
                            return;
                        }
                        accumulators
